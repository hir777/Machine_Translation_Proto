...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_threads", type=int, default=2,
                        help="the number of threads for processing tokenization. \n Default: 2   Valid range: 1 <= num_threads <= 8")
    ...
    if num_threads < 1 or num_threads > 8:
        print("The value num_threads %d is invalid. " % num_threads)
        print("It is replaced by %d." % 2)
        num_threads = 2

    print("\nTokenizing sentences...")
    start = time.time()
    tkn = tkn.Tokenization(threading=args.threading,
